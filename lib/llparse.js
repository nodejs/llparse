...
class CodeAPI {
  ...
  // TODO(indutny): do something with these long names
  store(field) {
  ...
  mulAdd(field, options) {
    return new internal.code.MulAdd(this[kPrefix] + '__muladd_' + field, field,
      options);
  }

  update(field, value) {
    return new internal.code.Update(this[kPrefix] + '__update_' + field, field,
      value);
  }

  isEqual(field, value) {
    return new internal.code.IsEqual(this[kPrefix] + '__is_eq_' + field, field,
      value);
  }
}
