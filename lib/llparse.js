...
class CodeAPI {
  ...
  store(field) {
    return new internal.code.Store(this[kPrefix] + '__load_' + field, field);
  }

  load(field) {
    return new internal.code.Load(this[kPrefix] + '__store_' + field, field);
  }
}

class Span {
  constructor(name, code, prefix) {
    this[kName] = name;
    this[kCode] = code;
    this[kPrefix] = prefix;
  }
  ...
  start() {
    return new internal.node.SpanStart(this[kName], this[kPrefix]);
  }

  end() {
    return new internal.node.SpanEnd(this[kName], this[kPrefix], this[kCode]);
  }
}
