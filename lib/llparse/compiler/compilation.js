...
class Compilation {
  constructor(options) {
    ...
    // Filled by `node.Translator`, maximum sequence length
    this.maxSequence = 0;
    ...
    this.buildStages(this.options.before);

    this.declareFields();

    this.buildStages(this.options.after);
  }
  ...
  declareFields() {
    ...
    // Find number of bits for an index field
    let width = 8;

    // Add extra bit, because LLVM IR expects signed offsets
    const max = this.maxSequence * 2;
    for (; Math.pow(2, width) <= max && width < 32; width += 8) {
      // No-op
    }
    assert(Math.pow(2, width) > this.maxSequence, 'Sequence length OOB');
    this.TYPE_INDEX = this.ir.i(width);
    this.declareField(this.TYPE_INDEX, 'index', type => type.v(0));
    ...
    this.options.properties.forEach((prop) => {
      // TODO(indutny): support non-int types?
      this.declareField(prop.type(this.ir, this.state), prop.name,
        type => type.v(0));
    });
  }
