...
class Allocator extends compiler.Stage {
  ...
  computeActive(nodes) {
    ...
    while (queue.size !== 0) {
      ...
      if (node instanceof llparse.compiler.node.SpanStart) {
      ...
      active.forEach((span) => {
        ...
        if (node instanceof llparse.compiler.node.SpanEnd &&
            span === this.id(node)) {
          return;
        }
    ...
    const ends = nodes
      .filter(node => node instanceof llparse.compiler.node.SpanEnd);
    ends.forEach((end) => {
