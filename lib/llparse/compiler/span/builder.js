...
class Builder extends compiler.Stage {
  ...
  build() {
    ...
    return {
      epilogue: (body) => this.buildEpilogue(body)
    };
  }
  ...
  buildEpilogue(body) {
    ...
    colors.concurrency.forEach((list, index) => {
      ...
      const startPtr = this.ctx.field(SPAN_START_PREFIX + index);

      // TODO(indutny): branch and call

      let cb;
      if (num === 1) {
        cb = this.buildCode(list[0]);
      } else {
        const cbPtr = this.ctx.field(SPAN_CB_PREFIX + index);
        body.push(cbPtr);

        cb = this.ctx.ir._('load', callbackType.to, [ callbackType, cbPtr ]);
        body.push(cb);
      }
    });
  }
