...
class NodeTranslator extends Stage {
  ...
  buildNode(node, value) {
    ...
    if (node instanceof llparse.node.Invoke) {
      ...
      res = new compiler.node.Consume(this.id(node), node.fieldName);
    } else {
    ...
    for (;;) {
      if (!(res instanceof compiler.node.Empty))
        break;
      if (res.skip)
        break;
  ...
  buildError(node) {
    ...
    const res = new compiler.node.Error(this.id(node), node.code, node.reason);
