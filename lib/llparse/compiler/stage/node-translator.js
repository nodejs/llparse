...
class NodeTranslator extends Stage {
  ...
  id(node, postfix = '') {
    ...
    return { name: 'n_' + res, sourceName: node.name };
  }
  ...
  buildSingle(node, trie, list, isRoot) {
    ...
    const first = trie.children[0];
    const isBitCheck =
      first.child.type === 'next' &&
      trie.children.length >= this.options.minCheckSize &&
      trie.children.every((child) => {
        if (child.child.value !== null)
          return false;
        if (child.noAdvance !== first.noAdvance)
          return false;

        if (child.child.type !== 'next')
          return false;

        return child.child.next === first.child.next;
      });

    // TODO(indutny): we can split a Single node if only small amount of
    // children leaves elsewhere
    // Fast case, every child leads to the same result and no values are passed
    if (isBitCheck)
      return this.buildBitCheck(node, trie, list, isRoot);
  ...
  buildBitCheck(node, trie, list, isRoot) {
    const keys = trie.children.map(child => child.key);
    const res = new compiler.node.BitCheck(this.id(node), keys);
    ...
    res.transform = node[kTransform];
    const next = trie.children[0];

    res.next = {
      node: this.buildTrie(node, next.child, list),
      noAdvance: next.noAdvance
    };
