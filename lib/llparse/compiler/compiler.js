...
class Compiler {
  ...
  buildExecute(ctx) {
    ...
    // TODO(indutny): remove `Invoke`, `Error`, and other nodes without a
    // prologue out of this list.
    const nodes = ctx.stageResults['node-builder'].list;
    const callees = ctx.ir.metadata(nodes.map((node) => {
      return node.type.ptr().type + ' @' + node.name;
    }).join(', '));
