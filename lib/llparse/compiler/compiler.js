...
class Compiler {
  constructor(options) {
    ...
    this.ctx = new llparse.compiler.Context(this.prefix, this.options);
  ...
  build(root) {
    ...
    const stages = [
      new llparse.compiler.MatchSequence(this.ctx),
      new llparse.compiler.span.Builder(this.ctx),
      new llparse.compiler.NodeBuilder(this.ctx)
    ];

    stages.forEach(stage => stage.build());
  ...
  buildSpanFields(root) {
    const allocator = new llparse.span.Allocator();

    const colors = allocator.execute(root);
  ...
  createFn(node) {
    ...
    if (node instanceof llparse.node.SpanStart ||
        node instanceof llparse.node.SpanEnd) {
      // no-op
    } else {
      // Prefix user-supplied node names
      name = 'n_' + name;
    }
