...
class Error extends node.Node {
  ...
  doBuild(ctx, body) {
    ...
    const cast = IR._('getelementptr inbounds', reason.type.to,
      [ reason.type, reason ], [ INT, INT.v(0) ], [ INT, INT.v(0) ]);
    body.push(cast);

    body.push(IR._('store', [ ctx.TYPE_ERROR, ctx.TYPE_ERROR.v(this.code) ],
      [ ctx.TYPE_ERROR.ptr(), codePtr ]).void());
    body.push(IR._('store', [ ctx.TYPE_REASON, cast ],
