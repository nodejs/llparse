...
class Node {
  ...
  build(compilation, nodes) {
    ...
    ctx.pos.next = ctx.ir._('getelementptr inbounds', ctx.pos.current.type.to,
  ...
  tailTo(ctx, body, pos, node, value = null) {
    const target = node.build(ctx.compilation, ctx.nodes);
    ...
    trampoline.name = body.name + '.trampoline';
    let phi = null;
    ...
    // Save state when tailing to Error node for future resumption
    if (node.type === 'error') {
      trampoline.comment('save current');

      const currentPtr = ctx.field('_current');
      trampoline.push(currentPtr);

      trampoline.push(ctx.ir._('store',
        [ ctx.fn.type.ptr(), ctx.fn ],
        [ ctx.fn.type.ptr().ptr(), currentPtr ]).void());
    }
  ...
  doOtherwise(ctx, body, pos) {
    ...
    this.tailTo(ctx, body, next, this.otherwise);
  }
}
module.exports = Node;
