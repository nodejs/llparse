...
class Compiler {
  constructor(prefix) {
    ...
    this.signature = {
      node: {
        match: this.ir.signature(TYPE_OUTPUT, [
          [ this.state.ptr(), ATTR_STATE ],
          [ TYPE_INPUT, ATTR_POS ],
          [ TYPE_INPUT, ATTR_ENDPOS ]
        ]),
        value: this.ir.signature(TYPE_OUTPUT, [
          [ this.state.ptr(), ATTR_STATE ],
          [ TYPE_INPUT, ATTR_POS ],
          [ TYPE_INPUT, ATTR_ENDPOS ],
          TYPE_MATCH
      ...
      store: this.ir.signature(INT, [
        this.state.ptr(), TYPE_INPUT, TYPE_INPUT, TYPE_MATCH
      ])
    };

    this.state.field(this.signature.node.match.ptr(), 'current');
  ...
  buildParse() {
    ...
    const nodeSig = this.signature.node.match;
  ...
  createFn(node) {
    ...
    if (node[kSignature] === 'match') {
      fn = this.ir.fn(this.signature.node.match, name,
        [ ARG_STATE, ARG_POS, ARG_ENDPOS ]);
    } else {
      assert.strictEqual(node[kSignature], 'value');
      fn = this.ir.fn(this.signature.node.value, name,
