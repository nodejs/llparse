...
class Compiler {
  constructor(prefix) {
    ...
    this.signature = {
      node: this.ir.signature(TYPE_OUTPUT, [
        [ this.state.ptr(), 'noalias nocapture nonnull' ],
        [ TYPE_INPUT, 'noalias nocapture nonnull readonly' ],
        [ TYPE_INPUT, 'noalias nocapture nonnull readnone' ]
      ]),
  ...
  createFn(node) {
    ...
    const fn = this.ir.fn(this.signature.node, name,
      [ ARG_STATE, ARG_POS, ARG_ENDPOS ]);
    fn.visibility = `internal ${CCONV}`;
    fn.attributes = 'nounwind optsize';
    return fn;
  }
  ...
  buildSelfReturn(fn, body, cache) {
    if (this.selfReturnCache.has(fn)) {
      const target = this.selfReturnCache.get(fn);
      body.terminate('br', target);
      return;
    }

    if (cache)
      this.selfReturnCache.set(fn, body);
