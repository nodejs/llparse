...
class Compiler {
  ...
  buildTrie(info, body, trie) {
    ...
    // Load the character
    const current = IR._('load', TYPE_INPUT.to, [ TYPE_INPUT, fn.arg(ARG_POS) ]);
    body.push(current);
    ...
    if (trie === null) {
      ...
      body = this.buildSingle(info, body, current, pos, trie.children);
    } else if (trie.type === 'sequence') {
      body = this.buildSequence(info, body, current, pos, trie);
    } else {
  ...
  buildSingle(info, body, current, pos, children) {
  ...
  buildSequence(info, body, current, pos, trie) {
    ...
    const expectedPtr = pos.next = IR._('getelementptr inbounds', seq.type.to,
        [ seq.type, seq ],
        [ I32, I32.v(0) ],
        [ I32, index ]);
    ...
    const complete = IR._('icmp', [ 'eq', TYPE_INDEX, index ],
    ...
    body.push(IR._('store', [ TYPE_INDEX, index ],
      [ TYPE_INDEX.ptr(), indexField ]).void());

    // TODO(indutny): just do a loop
    // Restart!
    this.buildRedirect(info, body, pos, info.fn);
