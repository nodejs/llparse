...
const directives = dsl.constants.directives;
...
class Compiler {
  ...
  buildCode() {
    ...
    this.globals.forEach((value, key) => {
      ...
      const builder = new Builder(this.reporter, this.state, this.settings,
        this.globals);
      value.build(builder);

      this.code.set(key, value);

      if (!builder.directives.isDefault)
        return;

      if (this.defaultState !== null) {
        return this.error(value.ast,
          `Two states with \`${directives.DEFAULT}\` directive`);
      }
    ...
    if (this.defaultState !== null)
      return;
    this.error(this.ast, `No state with \`${directives.DEFAULT}\` directive`);
  }

  serialize() {
    let out = '';

    out += this.state.serialize(this.prefix, this.defaultState);
    out += '\n';
    out += this.settings.serialize(this.prefix);
    out += '\n';
