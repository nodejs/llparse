...
class Compiler {
  ...
  compile() {
    ...
    if (this.state === null)
      this.error(this.ast, 'Missing global `state` declaration');

    if (this.settings === null)
      this.error(this.ast, 'Missing global `settings` declaration');
  ...
  parseTopLevel(body) {
    ...
    const declaration = (decl) => {
      ...
      if (decl.id.name === 'state')
        this.parseState(init);
      else if (decl.id.name === 'settings')
        this.parseSettings(init);
      else
        this.globals.define(decl.id, init, this);
    };
