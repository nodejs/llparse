...
class Builder {
  ...
  static build(reporter, node) {
    if (node.type === 'ArrowFunctionExpression')
      return new dsl.ir.Code(node);

    if (node.type === 'Literal') {
      const res = new dsl.ir.Constant(node.value);
      res.ast = node;
      return res;
    }
  ...
  processDirective(stmt) {
    if (stmt.directive === DIRECTIVE_DEFAULT)
  ...
  buildSwitch(stmt) {
    ...
    stmt.cases.forEach((c) => {
      ...
      this.block = block;
      for (let i = 0; i < c.consequent.length - 1; i++)
        this.buildStatement(c.consequent[i]);
      this.block = null;
    });
  }
  ...
  buildLiteral(literal) {
    return this.push(literal, new dsl.ir.Constant(literal.value));
  }
  ...
  buildAssignment(expr) {
    ...
    const index = this.state.lookup(prop);
    const right = this.buildExpression(expr.right);
    this.push(expr, new dsl.ir.StateStore(index, right));
    return right;
  }
